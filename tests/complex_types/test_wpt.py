...
class TestWpt:
    ...
    @pytest.mark.parametrize(
        "lat, lon, ele, time, magvar, geoidheight, name, cmt, desc, src, link, sym, type, fix, sat, hdop, vdop, pdop, ageofdgpsdata, dgpsid, extensions, tag",
        [
            pytest.param(
                Latitude(0),
                Longitude(1),
    ...
    def test_valid_values(
        ...
        assert all(isinstance(ll, Link) for ll in w.link)
        ...
        assert isinstance(w.ageofdgpsdata, float)
        ...
        assert w.cmt == "test-cmt"
        assert w.desc == "test-desc"
        assert w.src == "test-src"
        assert len(w.link) == 2
        assert w.sym == "test_sym"
        assert w.type == "test-type"
        assert w.fix.value == "none"
        assert w.sat == 42
        assert w.hdop == 0.1
        assert w.vdop == 0.2
        assert w.pdop == 0.3
        assert w.ageofdgpsdata == 0.4
    ...
    def test_default_values(self):
        ...
        assert w.ageofdgpsdata is None
